...
Some cache stores require asynchronous initialization, such as connecting to a remote database or service. To handle this, Cachimbo provides the `AsyncLazyCache` layer, which allows you to have a cache ready for use without having to deal with promises on construction.
...
```ts
import { AsyncLazyCache, RedisCache } from 'cachimbo';
...
```ts
import { AsyncLazyCache, LocalTTLCache } from 'cachimbo';
