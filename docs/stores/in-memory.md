...
|---------------------------------------------------------|------------------|-----------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|
| [Least Recently Used (LRU)](#least-recently-used-lru)   | Yes              | Yes, removes the least recently used item first     | When cached items have different access patterns and you want to keep the most recently accessed items cached.                |
| [Time To Live (TTL)](#time-to-live-ttl)                 | Yes              | Yes, removes the item closest to expiring first     | When cached items naturally expire after a fixed duration and access patterns are not a concern.                              |
...
If you don’t need access-pattern awareness, a TTL cache is faster, simpler, and often sufficient when data naturally expires after a fixed duration.
...
If you need neither access-pattern awareness nor a time-based expiration policy, you can use a simple Map-based cache. This is useful for testing or scenarios where you want to keep things extremely simple.
...
Be warned: The items can be deleted at an unpredictable time and order, as it all depends on the JavaScript engine's memory management and garbage collection process. Every read can result in a cache miss.
...
If you're using this layer together with a `WeakCache`, make sure that the `DeepCloningCache` layer is on top of the `WeakCache`, so that the weak references themselves are not cloned.
```ts
// This is correct:
const cache = new DeepCloningCache({
  cache: new WeakCache({
