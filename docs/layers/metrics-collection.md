...
To measure the cache effectiveness, you can add a `MetricsCollectingCache` layer. It collects how many times the cache was hit, missed, refreshed, updated and invalidated, along with the average time each of those took.
...
- The metrics are kept in memory, per instance. Use `getMetrics()` to read them and `resetMetrics()` to start over.
- The time of batch operations (`getMany`, `setMany` and `deleteMany`) is divided equally among all keys involved in the operation when calculating metrics.
